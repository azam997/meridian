...
class FFLogsClient:
    ...
    def get_character_kill_probe(
        self, lodestone_id: int,
        groups: list[tuple[int, int, list[int]]],
        spec_names: list[str],
        chunk: int = 5,
        on_chunk: Any = None,
    ) -> dict[str, list[dict[str, Any]]]:
        """Which of `spec_names` this character has ranked kills with, in as
        few round trips as possible: one query per chunk of specs, aliasing a
        `zoneRankings` per (spec, zone group) with the spec inlined as a
        literal (`s{i}z{zone}:`). specName is a required argument on every
        character rankings query, so a per-spec fan-out is unavoidable —
        chunking amortizes it, and a GraphQL complexity error (FFLogs'
        per-request point ceiling, same reason `_SUMMARY_BATCH` exists)
        halves the chunk and retries, flooring at 1 spec/request — the
        proven `get_character_setup` shape.

        Returns `{spec_name: encounters}` for every requested spec, where
        `encounters` is the `_encounters_from_zone_rankings` shape merged in
        group order (empty list = no ranked kill this tier). `on_chunk(done,
        total)` fires after each successful chunk, for progress emission."""
        out: dict[str, list[dict[str, Any]]] = {}
        pending = list(spec_names)
        total = len(pending)
        size = max(1, int(chunk))
        while pending:
            batch = pending[:size]
            try:
                out.update(self._kill_probe_chunk(lodestone_id, groups, batch))
            except RuntimeError:
                if size > 1:
                    size = max(1, size // 2)
                    continue
            ...
            if on_chunk is not None:
                on_chunk(total - len(pending), total)
        return out

    def _kill_probe_chunk(
        self, lodestone_id: int,
        groups: list[tuple[int, int, list[int]]],
        batch: list[str],
    ) -> dict[str, list[dict[str, Any]]]:
        fields = "\n              ".join(
            f's{si}z{int(zid)}: zoneRankings(zoneID: {int(zid)}, '
            f'specName: "{fflogs_spec_slug(spec)}", difficulty: {int(diff)})'
        ...
        for si, spec in enumerate(batch):
            ...
            if char:
                for zid, _, _ in groups:
                    encs.extend(_encounters_from_zone_rankings(
                        char.get(f"s{si}z{int(zid)}") or {}))
            out[spec] = encs
        return out
