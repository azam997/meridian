...
def list_prog_pulls(req: dict) -> dict:
    ...
    pulls: list[dict] = []
    for code, summary, f in _iter_wipe_fights(client, codes, {encounter_id},
                                              strict=(source == "report")):
        jobs_in = {str(j).replace(" ", "")
                   for j in party_jobs_in_fight(summary, f)}
        if want_job not in jobs_in:
            continue
        pulls.append(_serialize_prog_pull(code, f, summary.get("startTime") or 0))
    pulls.sort(key=lambda p: p["startTimeMs"], reverse=True)
    return {"pulls": pulls[:_PROG_MAX_PULLS], "source": source}


def _iter_wipe_fights(client, codes: list[str], encounter_ids: set[int],
                      *, strict: bool = False):
    """Yield `(code, summary, fight)` for every wipe ≥ `_PROG_MIN_DURATION_S`
    on one of `encounter_ids` across these report summaries. The shared core
    of `list_prog_pulls` (one encounter, party-level job filter) and
    `list_character_pulls` (all tier encounters, identity-attributed).
    Batched prefetch + tolerant per-code fetch; `strict=True` raises on a
    missing report — the pasted-code mode, where silence would read as
    "no pulls"."""
    try:
    ...
    for code in codes:
        ...
        for f in summary.get("fights") or []:
            if f.get("encounterID") not in encounter_ids:
                continue
            if f.get("kill") is not False:
                continue
            if (f["endTime"] - f["startTime"]) / 1000.0 < _PROG_MIN_DURATION_S:
                continue
            yield code, summary, f
...
# --- list_character_pulls: the job-agnostic Pulls screen -------------------

# Actor subType arrives spaceless ("RedMage") or spaced depending on the
# surface; internal job names keep the space. Normalize through the slug.
_JOB_BY_SLUG = {j.replace(" ", ""): j for j in ALL_JOBS}

# Handler-level session memo: the merged list is a fan-out (job probe + per-
# spec setups + recent-report scan), so repeat visits to the Pulls screen hit
# this instead. Keyed (lodestoneId, recentLimit); forceRefresh (the Refresh
# button) skips and replaces. Small cap — one entry per character in practice.
_CHAR_PULLS_CACHE_CAP = 4
_char_pulls_cache: "OrderedDict[tuple, dict]" = OrderedDict()
_char_pulls_lock = threading.Lock()


def _actor_job_for_character(summary: dict, fight: dict,
                             name: str, server: str) -> str | None:
    """The job THIS character played in a fight, or None if they're not in it.

    Rankings rows are character-scoped by construction, but report summaries
    only know actors, and actors carry no lodestoneID — case-insensitive
    name (+ server, when both sides have one) is the only identity join.
    Returns the internal spaced job name ("Red Mage")."""
    want_name = (name or "").strip().lower()
    if not want_name:
        return None
    want_server = (server or "").strip().lower()
    actors = ((summary or {}).get("masterData") or {}).get("actors") or []
    friendly = set(fight.get("friendlyPlayers") or [])
    for a in actors:
        if a.get("type") != "Player" or a.get("id") not in friendly:
            continue
        if str(a.get("name") or "").strip().lower() != want_name:
            continue
        a_server = str(a.get("server") or "").strip().lower()
        if want_server and a_server and a_server != want_server:
            continue
        slug = str(a.get("subType") or "").replace(" ", "")
        return _JOB_BY_SLUG.get(slug)
    return None


def _serialize_pull_row(*, report_code: str, fight_id: int, encounter_id: int,
                        job: str, kill: bool, start_time_ms: int,
                        duration_s: float, parse_pct: float | None = None,
                        dps: float | None = None,
                        fight_percentage: float | None = None,
                        boss_percentage: float | None = None,
                        last_phase: int | None = None) -> dict:
    """One unified Pulls-screen row. Kill rows carry parse/dps (from rankings);
    wipe rows carry phase/progress (from summaries); the rest is None — the
    row itself says which via `kill`. Labels/meta lines are built client-side."""
    return {
...
def list_character_pulls(req: dict, req_id: str) -> dict:
    """Everything the Pulls screen needs for one character, job-agnostic:
    every tier pull (ranked kills across ALL jobs + wipes from recent
    reports), merged newest-first, plus the encounter catalog and the jobs
    seen. Three passes, each progress-emitted (the cold path is several
    round trips and must keep the frontend's idle timeout fed):

      1. kill probe — which specs have ranked kills (chunked aliased query);
      2. per hit spec, the proven one-shot setup query (session-memoized);
      3. recent-report wipe scan, identity-attributed via name+server.

    `reportCode` switches to the pasted-report mode instead (one report,
    both kills and wipes, no rankings)."""
    client = _client()
    name = (req.get("characterName") or "").strip()
    server = (req.get("server") or "").strip()
    pasted = (req.get("reportCode") or "").strip()
    if pasted:
        return _character_pulls_from_report(client, pasted, name, server)

    lodestone_id = req["lodestoneId"]
    recent_limit = int(req.get("recentLimit") or _PROG_RECENT_REPORTS)
    force = bool(req.get("forceRefresh"))
    cache_key = (lodestone_id, recent_limit)
    if not force:
        with _char_pulls_lock:
            hit = _char_pulls_cache.get(cache_key)
            if hit is not None:
                _char_pulls_cache.move_to_end(cache_key)
                return hit

    def progress(pct: int, stage: str) -> None:
        _emit({"id": req_id, "progress": {"stage": stage, "pct": pct}})

    # Pass 1 — which jobs this character has ranked kills with.
    progress(2, "Scanning jobs")
    probe = client.get_character_kill_probe(
        lodestone_id, ZONE_GROUPS, ALL_JOBS,
        on_chunk=lambda done, total: progress(
            2 + int(28 * done / max(1, total)),
            f"Scanning jobs ({done}/{total})"))
    hit_specs = [s for s in ALL_JOBS if probe.get(s)]

    # Pass 2 — full ranked pulls per hit spec (typically 1-3 specs; each is
    # the memoized `get_character_setup` one-shot, so a job the user then
    # analyzes costs nothing extra).
    rows: list[dict] = []
    enc_names: dict[int, str] = {}
    kills_by_enc: dict[int, int] = {}
    for i, spec in enumerate(hit_specs):
        progress(30 + int(42 * i / max(1, len(hit_specs))),
                 f"Fetching {spec} kills")
        setup = client.get_character_setup(
            lodestone_id=lodestone_id, groups=ZONE_GROUPS, spec_name=spec)
        for eid, plist in (setup.get("pulls") or {}).items():
            for p in plist:
                rows.append(_serialize_pull_row(
                    report_code=p["report_code"], fight_id=p["fight_id"],
                    encounter_id=int(eid), job=spec, kill=True,
                    start_time_ms=p["start_time_ms"],
                    duration_s=p["duration_s"],
                    parse_pct=p.get("parse_pct"), dps=p.get("dps")))
    for spec, encs in probe.items():
        for e in encs:
            kills_by_enc[e["id"]] = (kills_by_enc.get(e["id"], 0)
                                     + (e.get("total_kills") or 0))
            enc_names.setdefault(e["id"], e["name"])

    # Pass 3 — wipes from recent reports, attributed to THIS character.
    progress(74, "Scanning recent reports")
    reports = client.get_character_recent_reports(lodestone_id,
                                                  limit=recent_limit)
    tier_zones = {zid for zid, _, _ in ZONE_GROUPS}
    in_zone = [r for r in reports if r.get("zone_id") in tier_zones]
    # Same fallback as list_prog_pulls: a report's zone is its primary zone,
    # so an empty pre-filter means scan everything rather than miss wipes.
    codes = list(dict.fromkeys(r["code"] for r in (in_zone or reports)))
    all_eids = {eid for _, _, eids in ZONE_GROUPS for eid in eids}
    for code, summary, f in _iter_wipe_fights(client, codes, all_eids):
        job = _actor_job_for_character(summary, f, name, server)
        if not job:
            continue
        rows.append(_serialize_pull_row(
            report_code=code, fight_id=f["id"],
            encounter_id=f.get("encounterID"), job=job, kill=False,
            start_time_ms=(summary.get("startTime") or 0)
                          + (f.get("startTime") or 0),
            duration_s=(f["endTime"] - f["startTime"]) / 1000.0,
            fight_percentage=f.get("fightPercentage"),
            boss_percentage=f.get("bossPercentage"),
            last_phase=f.get("lastPhase") or 0))

    # Merge: dedupe on (report, fight) — a kill row wins over a wipe row for
    # the same fight (richer data; can only collide via pasted merges anyway).
    seen: dict[tuple, dict] = {}
    for r in rows:
        key = (r["reportCode"], r["fightId"])
        prev = seen.get(key)
        if prev is None or (r["kill"] and not prev["kill"]):
            seen[key] = r
    merged = sorted(seen.values(), key=lambda r: r["startTimeMs"],
                    reverse=True)

    # Full catalog (kill counts summed across jobs), FFLogs names where the
    # character has kills, catalog names otherwise — same synthesis idea as
    # list_setup's zero-kill ultimates, extended to the whole tier.
    encounters = [{
        "id": eid,
        "name": enc_names.get(eid, cat_name),
        "totalKills": kills_by_enc.get(eid, 0),
        "category": encounter_category(eid),
    } for eid, cat_name in ALL_ENCOUNTERS]

    jobs_seen = {r["job"] for r in merged}
    result = {
        "pulls": merged,
        "encounters": encounters,
        "jobs": [j for j in ALL_JOBS if j in jobs_seen],
        "syncedAtMs": int(time.time() * 1000),
        "recentLimit": recent_limit,
    }
    with _char_pulls_lock:
        _char_pulls_cache[cache_key] = result
        _char_pulls_cache.move_to_end(cache_key)
        while len(_char_pulls_cache) > _CHAR_PULLS_CACHE_CAP:
            _char_pulls_cache.popitem(last=False)
    return result


def _character_pulls_from_report(client, code: str, name: str,
                                 server: str) -> dict:
    """Pasted-report mode: THIS character's tier fights in one report — kills
    and wipes, no rankings involved. Kills found this way carry no parse/dps
    (summaries have neither); the UI falls back to duration-only. Raises when
    the report is missing or holds no fights by the character, so the search
    box can show a reason instead of silently merging nothing."""
    try:
        summary = client.get_report_summary(code)
    except Exception:
        summary = None
    if not summary:
        raise RuntimeError(f"Report {code} not found on FFLogs")
    all_eids = {eid for _, _, eids in ZONE_GROUPS for eid in eids}
    report_start = summary.get("startTime") or 0
    rows: list[dict] = []
    for f in summary.get("fights") or []:
        if f.get("encounterID") not in all_eids:
            continue
        kill_flag = f.get("kill")
        if kill_flag is not True and kill_flag is not False:
            continue
        dur_s = (f["endTime"] - f["startTime"]) / 1000.0
        if kill_flag is False and dur_s < _PROG_MIN_DURATION_S:
            continue
        job = _actor_job_for_character(summary, f, name, server)
        if not job:
            continue
        rows.append(_serialize_pull_row(
            report_code=code, fight_id=f["id"],
            encounter_id=f.get("encounterID"), job=job, kill=kill_flag,
            start_time_ms=report_start + (f.get("startTime") or 0),
            duration_s=dur_s,
            fight_percentage=None if kill_flag else f.get("fightPercentage"),
            boss_percentage=None if kill_flag else f.get("bossPercentage"),
            last_phase=None if kill_flag else (f.get("lastPhase") or 0)))
    if not rows:
        who = name or "your character"
        raise RuntimeError(f"No tier pulls by {who} in report {code}")
    rows.sort(key=lambda r: r["startTimeMs"], reverse=True)
    return {
        "pulls": rows,
        "encounters": [],
...
HANDLERS = {
    ...
    "list_character_pulls": lambda req, _id: {"ok": True, "data": list_character_pulls(req, _id)},
