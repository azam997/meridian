"""White Mage simulator validation against REAL pulls (no network at test time).

The companion to test_whitemage_sim.py (internal invariants). This one
validates the sim against actual human play — real quartile-stratified WHM
pulls captured from FFLogs top rankings (one per quartile per encounter),
under tests/fixtures/whm/. Validating against the sim's own output would be
circular; these are real cast streams, so the tests confirm the sim correlates
with skill and never under-rates a real top parse.

Healer note: a real WHM spends GCDs healing (Cure III / Medica III / Regen …),
which is genuine damage-efficiency loss vs the ceiling — so even top-quartile
healers sit lower than top DPS do. The phase-continuation entry lily state is
measured from the fixture's own cast stream (deepest-deficit), so M12S-P2
style fixtures seed the ceiling symmetrically.

Regenerate / add encounters with:
    python scripts/add_whitemage_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_whitemage_pulls.py
"""
...
class MockClient:
    """Serves a real captured WHM pull (casts + targetability + comp).
    No buff stream in the fixtures -> no observed raid buffs / tincture
    windows (the strict scenario is the assertion target anyway)."""
        ...
        return {
            ...
            "masterData": {
                "actors": [{
                    ...
                    "subType": "WhiteMage", "petOwner": None, "gameID": 24,
                }, *other, *npc_actors],
            },
        }
...
def _fixture_names() -> list[str]:
    return [p.stem for p in sorted(WHM_FIXTURES_DIR.glob("*.json"))
            if p.stem != "synthetic"]
...
def _analyze(name: str):
    fix = json.loads((WHM_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("White Mage", client, fix["report_code"], fix["fight_id"],
...
_WHM_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no WHM pull fixtures")
@pytest.mark.parametrize("name", _FIXTURE_NAMES)
def test_pull_invariants(name: str) -> None:
    """Per real pull: pipeline runs, every aspect present, delivered potency in
    a sane p/sec band, and idealized >= delivered within the tolerance."""
    mr, fix = _analyze(name)
    for aname in _WHM_ASPECTS:
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (WHM_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no WHM pull fixtures — run scripts/add_whitemage_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll WHM real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
