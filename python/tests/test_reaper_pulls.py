"""Reaper simulator validation against REAL pulls (no network at test time).

The companion to test_reaper_sim.py (which checks the simulator's internal
invariants). This one validates the sim against actual human play — real
quartile-stratified Reaper pulls captured from FFLogs top rankings (one per
quartile per encounter), under tests/fixtures/rpr/. Validating against the sim's
own output would be circular; these are real cast streams, so the tests confirm
the sim correlates with skill and never under-rates a real top parse.

Death's Design coverage: the captured fixtures carry no DamageDone stream, so
`measured_dd_intervals` falls back to FULL coverage — the x1.10 amp then cancels
in delivered/idealized (accurate for top pulls, which hold DD ~100%).

Regenerate / add encounters with:
    python scripts/add_reaper_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_reaper_pulls.py
"""
...
RPR_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "rpr"
_BUCKETS = ("topq", "q2", "q3", "botq")


class MockClient:
    """Serves a real captured RPR pull (casts + targetability + comp), mirroring
    test_redmage_pulls.py's MockClient. Returns [] for the DamageDone stream so
    Death's Design falls back to full coverage. No network."""
...
def _fixture_names() -> list[str]:
    return [p.stem for p in sorted(RPR_FIXTURES_DIR.glob("*.json"))]
...
def _analyze(name: str):
    fix = json.loads((RPR_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Reaper", client, fix["report_code"], fix["fight_id"],
...
_RPR_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring", "DeathsDesign"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no RPR pull fixtures")
@pytest.mark.parametrize("name", _FIXTURE_NAMES)
def test_pull_invariants(name: str) -> None:
    """Per real pull: pipeline runs, every aspect present, delivered potency in a
    sane p/sec band, and idealized >= delivered (the same 0.5% headroom MCH/RDM
    use). The pre-pull Harpe sweep + the melee engage delay keep the ceiling at
    or above a real top parse rather than letting it edge over."""
    mr, fix = _analyze(name)
    for aname in _RPR_ASPECTS:
    ...
    assert 150 <= pps <= 600, f"{name}: p/sec {pps:.1f} out of band"
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (RPR_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no RPR pull fixtures — run scripts/add_reaper_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll RPR real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
