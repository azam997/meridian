"""Summoner simulator validation against REAL pulls (no network at test time).

The companion to test_summoner_sim.py (internal invariants). This one validates
the sim against actual human play — real quartile-stratified SMN pulls captured
from FFLogs top rankings (one per quartile per encounter), under
tests/fixtures/smn/. Validating against the sim's own output would be circular;
these are real cast streams, so the tests confirm the sim correlates with skill
and never under-rates a real top parse.

SMN is RNG-free — the ceiling is pure (duration, downtime) data plus the
per-player GCD, so no per-pull budget context is needed (the pet contributions
are folded onto the player's own cast ids).

Regenerate / add encounters with:
    python scripts/add_summoner_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_summoner_pulls.py
"""
...
SMN_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "smn"
...
class MockClient:
    """Serves a real captured SMN pull (casts + targetability + comp). Returns []
    for Buffs / DamageDone (no observed tincture/raid windows — the strict lens
    under test is buff-agnostic anyway). No network, no pet actors — the pet
    folds ride the player cast ids."""
...
def _fixture_names() -> list[str]:
    if not SMN_FIXTURES_DIR.exists():
        return []
    return [p.stem for p in sorted(SMN_FIXTURES_DIR.glob("*.json"))
            if p.stem != "synthetic"]
...
def _analyze(name: str):
    fix = json.loads((SMN_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Summoner", client, fix["report_code"], fix["fight_id"],
...
_SMN_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no SMN pull fixtures")
...
def test_pull_invariants(name: str) -> None:
    ...
    assert 150 <= pps <= 400, f"{name}: p/sec {pps:.1f} out of band"
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (SMN_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no SMN pull fixtures — run scripts/add_summoner_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll SMN real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
