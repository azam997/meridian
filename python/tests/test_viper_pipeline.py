"""VPR end-to-end pipeline + contract snapshot test.

Drives `analyze_pull` and the sidecar's `_build_response` against the VPR synthetic
fixture (no FFLogs network calls) and compares the JSON output to a frozen snapshot.
Complements `test_viper_sim.py` (internal invariants) and `test_viper_pulls.py`
...
When the contract intentionally changes, regenerate with:
    UPDATE_SNAPSHOT=1 python tests/test_viper_pipeline.py

Run from python/:
    python tests/test_viper_pipeline.py
"""
...
FIXTURE_DIR = Path(__file__).parent / "fixtures" / "vpr"
...
class StubFFLogsClient:
    """Serves the VPR synthetic fixture. Refs flow is short-circuited via empty
    rankings — the snapshot covers the VPR `you` pipeline + the no-refs branches.
    DamageDone / Buffs return [] so Hunter's Instinct falls back to full coverage."""
    ...
    def get_report_summary(self, code: str) -> dict:
        f = self._fixture
        return {
            "title": f.get("label", "VPR synthetic"),
            "startTime": f["fight_start_ms"],
            "endTime": f["fight_end_ms"],
            "fights": [{
                "id": f["fight_id"],
                "name": "Synthetic VPR Fight",
                "encounterID": 101,
                "difficulty": 101,
                "kill": True,
                "startTime": f["fight_start_ms"],
            ...
            "masterData": {
                "actors": [
                    {
                        "id": f["source_id"],
                        "name": f.get("label", "VPR Synthetic Player"),
                        "server": "TestServer",
                        "type": "Player",
                        "subType": "Viper",
                        "petOwner": None,
...
def _run_pipeline() -> dict:
    """Drive analyze_pull + _build_response against the VPR fixture without
    ...
    you = analyze_pull(
        "Viper", stub, fixture["report_code"], fixture["fight_id"],
        ranking_name=None, label="You",
    )
    refs: list = []
    comparisons = _compare_all_aspects("Viper", you, refs)
    return _build_response("Viper", you, refs, comparisons)
...
def test_vpr_response_shape(response: dict) -> None:
    """VPR ships with only the shared aspects — confirm the response has
...
def test_vpr_headline_has_efficiency(response: dict) -> None:
    """VPR ships a full simulator → a real idealized ceiling + efficiency."""
...
def test_vpr_overcap_findings_known_gauges(response: dict) -> None:
    """VPR has 2 overcap gauges (offering, rattling); any findings reference only
    those."""
    overcap = response.get("aspectStates", {}).get("Overcap", {})
    findings = overcap.get("findings") or []
    valid = {"offering", "rattling"}
    bad = [f.get("gauge") for f in findings if f.get("gauge") not in valid]
    _check("no findings on unknown gauges", not bad, f"got {bad}")


def test_vpr_ability_meta_only_vpr_ids(response: dict) -> None:
    """abilityMeta should reference only VPR-related ids (fixture casts ∪ the VPR
    potency / cooldown / defensive tables) — no leftover ids from another job."""
    meta = response.get("abilityMeta", {})
    fixture = json.loads(
        (FIXTURE_DIR / f"{FIXTURE_NAME}.json").read_text(encoding="utf-8"))
    from jobs.viper.data import COOLDOWNS, JOB_DATA, POTENCIES
    ...
    for aid_str in meta.keys():
        aid = int(aid_str)
        _check(f"abilityMeta[{aid}] is a VPR-related id", aid in allowed,
               f"got {aid}, not a VPR ability")


# --- Snapshot ---------------------------------------------------------------

def _snapshot_path() -> Path:
    return SNAPSHOTS_DIR / f"vpr_{FIXTURE_NAME}.snapshot.json"


def test_vpr_snapshot(actual: dict) -> None:
    ...
    if not snap_path.exists():
        _check("snapshot exists", False,
               ...
               f"UPDATE_SNAPSHOT=1 python tests/test_viper_pipeline.py")
        return
    ...
    for k in sorted(set(actual.keys()) | set(expected.keys())):
        ...
        if a_str != e_str:
            ...
            if mismatches >= 3:
                print("         ...")
                break
    print("         run: UPDATE_SNAPSHOT=1 python tests/test_viper_pipeline.py")
    raise AssertionError("snapshot mismatch")


def main() -> int:
    print(f"Loading VPR fixture: {FIXTURE_NAME}")
    actual = _normalize(_run_pipeline())
    if os.environ.get("UPDATE_SNAPSHOT") == "1":
        test_vpr_snapshot(actual)
        print("Snapshot written.")
        return 0
    test_vpr_response_shape(actual)
    test_vpr_headline_has_efficiency(actual)
    test_vpr_overcap_findings_known_gauges(actual)
    test_vpr_ability_meta_only_vpr_ids(actual)
    test_vpr_snapshot(actual)
