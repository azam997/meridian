"""SGE end-to-end pipeline + contract snapshot test.

Drives `analyze_pull` and the sidecar's `_build_response` against the SGE synthetic
fixture (no FFLogs network calls) and compares the JSON output to a frozen snapshot.
SGE is the fourth *healer*; this locks the multi-job contract shape for the healer
path the same way test_scholar_pipeline.py does.

The fixture is the deterministic output of scripts/gen_sage_synthetic_fixture.py
(default-sim rotation with a deliberate one-cast Psyche drift).

When the contract intentionally changes, regenerate with:
    UPDATE_SNAPSHOT=1 python tests/test_sage_pipeline.py

Run from python/:
    python tests/test_sage_pipeline.py
"""
...
class StubFFLogsClient:
    """Mirrors the SCH stub. Refs flow short-circuited via empty rankings — the
    snapshot covers the SGE `you` pipeline + the no-refs comparison."""
    ...
    def get_report_summary(self, code: str) -> dict:
        f = self._fixture
        return {
            "title": f.get("label", "SGE synthetic"),
            "startTime": f["fight_start_ms"],
            "endTime": f["fight_end_ms"],
            "fights": [{
                "id": f["fight_id"],
                "name": "Synthetic SGE Fight",
                "encounterID": 101,
                "difficulty": 101,
                "kill": True,
                "startTime": f["fight_start_ms"],
            ...
            "masterData": {
                "actors": [
                    {
                        "id": f["source_id"],
                        "name": f.get("label", "SGE Synthetic Player"),
                        "server": "TestServer",
...
def _run_pipeline() -> dict:
    ...
    you = analyze_pull(
        "Sage", stub, fixture["report_code"], fixture["fight_id"],
        ranking_name=None, label="You",
    )
    refs: list = []
    comparisons = _compare_all_aspects("Sage", you, refs)
    return _build_response("Sage", you, refs, comparisons)
...
def test_sge_response_shape(response: dict) -> None:
    """SGE ships with only the shared aspects — confirm the response has them and
    none of the other jobs' bespoke aspects."""
    print()
    print("Test: SGE aspectStates shape")
...
def test_sge_headline_has_efficiency(response: dict) -> None:
    """SGE ships a full simulator → a real idealized ceiling + efficiency."""
    print()
    print("Test: SGE headline carries a real efficiency")
...
def test_sge_drift_finds_psyche(response: dict) -> None:
    """The fixture casts Psyche once and never again across 360s — DriftAspect must
    flag the 60s cooldown sitting capped, with quantized lost potency > 0."""
    print()
    print("Test: SGE Drift finds the Psyche cooldown waste")
    drift = response.get("aspectStates", {}).get("Drift", {})
    findings = drift.get("findings") or []
    ps = next((f for f in findings if f.get("abilityId") == 37033), None)
    _check("Psyche drift finding present", ps is not None,
           f"got findings for {[f.get('abilityId') for f in findings]}")
    if ps:
        _check("Psyche capped_seconds > 60",
               ps.get("cappedSeconds", 0) > 60,
               f"got {ps.get('cappedSeconds')}")


def test_sge_ability_meta_only_sge_ids(response: dict) -> None:
    """abilityMeta should reference only SGE-related ids (fixture casts ∪ the SGE
    data tables — the idealized track can add abilities never cast)."""
    print()
    print("Test: SGE abilityMeta references SGE IDs only")
    meta = response.get("abilityMeta", {})
    fixture = json.loads(
        (FIXTURE_DIR / f"{FIXTURE_NAME}.json").read_text(encoding="utf-8")
    )
    from jobs.sage.data import (COOLDOWNS as SGE_COOLDOWNS, DEFENSIVE_IDS,
                                POTENCIES as SGE_POTENCIES)
    sge_ids = {ev["abilityGameID"] for ev in fixture["cast_events"]}
    allowed = (sge_ids | set(SGE_COOLDOWNS.keys()) | set(SGE_POTENCIES.keys())
               | set(DEFENSIVE_IDS))
    for aid_str in meta.keys():
        aid = int(aid_str)
        _check(f"abilityMeta[{aid}] is a SGE-related ID",
               aid in allowed,
               f"got {aid}, not in fixture casts or SGE tables")


# --- Snapshot ---------------------------------------------------------------

def _snapshot_path() -> Path:
    return SNAPSHOTS_DIR / f"sge_{FIXTURE_NAME}.snapshot.json"


def test_sge_snapshot(actual: dict) -> None:
    """Lock the camelized JSON response shape against a frozen snapshot."""
    print()
    print(f"Test: SGE contract snapshot for fixture {FIXTURE_NAME!r}")
    ...
    if not snap_path.exists():
        _check("snapshot exists", False,
               ...
               f"run UPDATE_SNAPSHOT=1 python tests/test_sage_pipeline.py")
        return
    ...
    for k in sorted(set(actual.keys()) | set(expected.keys())):
        ...
        if a_str != e_str:
            ...
            if mismatches >= 3:
                print("         ...")
                break
    print("         run: UPDATE_SNAPSHOT=1 python tests/test_sage_pipeline.py")
...
def main() -> int:
    print(f"Loading SGE fixture: {FIXTURE_NAME}")
    actual = _normalize(_run_pipeline())

    if os.environ.get("UPDATE_SNAPSHOT") == "1":
        test_sge_snapshot(actual)
        print()
        print("=" * 60)
        print("Snapshot written.")
        return 0

    test_sge_response_shape(actual)
    test_sge_headline_has_efficiency(actual)
    test_sge_drift_finds_psyche(actual)
    test_sge_ability_meta_only_sge_ids(actual)
    test_sge_snapshot(actual)
