"""Paladin simulator validation against REAL pulls (no network at test time).

The companion to test_paladin_sim.py (which checks the simulator's internal
invariants). This one validates the sim against actual human play — real
quartile-stratified Paladin pulls captured from FFLogs top rankings (one per
quartile per encounter), under tests/fixtures/pld/. Validating against the sim's
...
Regenerate / add encounters with:
    python scripts/add_paladin_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_paladin_pulls.py
"""
...
PLD_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "pld"
_BUCKETS = ("topq", "q2", "q3", "botq")


class MockClient:
    """Serves a real captured PLD pull (casts + targetability + comp), mirroring
    test_reaper_pulls.py's MockClient. No network."""
...
def _fixture_names() -> list[str]:
    return [p.stem for p in sorted(PLD_FIXTURES_DIR.glob("*.json"))]
...
def _analyze(name: str):
    fix = json.loads((PLD_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Paladin", client, fix["report_code"], fix["fight_id"],
...
_PLD_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no PLD pull fixtures")
@pytest.mark.parametrize("name", _FIXTURE_NAMES)
def test_pull_invariants(name: str) -> None:
    """Per real pull: pipeline runs, every aspect present, delivered potency in a
    sane p/sec band, and idealized >= delivered (the same 0.5% headroom the other
    sims use)."""
    mr, fix = _analyze(name)
    for aname in _PLD_ASPECTS:
    ...
    assert 120 <= pps <= 500, f"{name}: p/sec {pps:.1f} out of band"
    ideal = st["idealized_potency"]
    ratio = delivered / ideal if ideal > 0 else 0
    # 1% headroom (vs the 0.5% the other sims use). Calibrated against the live
    # PLD top-10 sweep across all 5 encounters (mean 98.9%, max 100.8%): with
    # authoritative potencies, the residual on the very best short-fight parses is
    # the greedy-vs-theoretical-optimum gap — world-top players sequence the
    # Atonement chain into the Fight-or-Flight window ~1% tighter than a greedy
    # priority sim packs it. Accepted rather than overfit with a fragile opener
    # heuristic; revisit if a true rotation optimizer lands.
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (PLD_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no PLD pull fixtures — run scripts/add_paladin_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll PLD real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
