"""Pictomancer simulator validation against REAL pulls (no network at test time).

The companion to test_pictomancer_sim.py (internal invariants). This one
validates the sim against actual human play — real quartile-stratified PCT
pulls captured from FFLogs top rankings (one per quartile per encounter), under
tests/fixtures/pct/. Validating against the sim's own output would be circular;
these are real cast streams, so the tests confirm the sim correlates with skill
and never under-rates a real top parse.

PCT is RNG-free — the ceiling is pure (duration, downtime) data plus the
per-player GCD, so no per-pull budget context is needed.

Regenerate / add encounters with:
    python scripts/add_pictomancer_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_pictomancer_pulls.py
"""
...
PCT_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "pct"
...
class MockClient:
    """Serves a real captured PCT pull (casts + targetability + comp). Returns []
        ...
        return {
            ...
            "masterData": {
                "actors": [{
                    ...
                    "subType": "Pictomancer", "petOwner": None, "gameID": 42,
                }, *other, *npc_actors],
            },
        }
...
def _fixture_names() -> list[str]:
    if not PCT_FIXTURES_DIR.exists():
        return []
    return [p.stem for p in sorted(PCT_FIXTURES_DIR.glob("*.json"))
            if p.stem != "synthetic"]
...
def _analyze(name: str):
    fix = json.loads((PCT_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Pictomancer", client, fix["report_code"], fix["fight_id"],
...
_PCT_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no PCT pull fixtures")
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (PCT_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no PCT pull fixtures — run scripts/add_pictomancer_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll PCT real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
