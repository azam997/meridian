"""SCH mit-plan locked-GCD integration (network-free).

SCH inherits the base-model lock hooks (Concitation is unconditionally
castable — the AST pattern, unlike WHM's resource-gated lily substitution).
Pins the inherited path end-to-end: a locked Concitation fires in its window,
an uptime lock costs about one Broil, a downtime lock is free, and locked
ceilings are monotone.

Run from python/:  python tests/test_scholar_locks.py
"""
...
from jobs.scholar.simulator import simulate_idealized
...
def test_uptime_lock_costs_about_one_filler() -> None:
    ...
    # One heal GCD displaces about one Broil (380p); allow DoT/oGCD re-timing.
    assert 150.0 <= drop <= 800.0, drop
...
def main() -> None:
    ...
    print("test_scholar_locks: all checks passed")
