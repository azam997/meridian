"""Pictomancer multi-target ceiling (the "II" swap layer).

PCT swaps the RGB/CMY subtractive fillers to their "II" AoE forms under a
multi-target schedule (`_RGB_AOE_SWAP` / `_CMY_AOE_SWAP`, with `_AOE_TO_BASE`
folding transitions back). Pins the universal guards: N==1 byte-identity, a
swap id appearing once the schedule engages, the never-worse invariant, and
the high-N ceiling lift — the `_AOE_TO_BASE` fold is exactly the mapping that
silently rots when an id changes.

Run from python/:  python tests/test_pictomancer_aoe.py
"""
...
from jobs.pictomancer import scoring
from jobs.pictomancer import simulator as sim

_DUR = 120.0
_SWAP_IDS = set(sim._RGB_AOE_SWAP.values()) | set(sim._CMY_AOE_SWAP.values())
...
def main() -> None:
    ...
    print("pictomancer_aoe: all checks passed")
