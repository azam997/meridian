"""GNB end-to-end pipeline + contract snapshot test.

Drives `analyze_pull` and the sidecar's `_build_response` against a synthetic
GNB pull (the default-sim timeline replayed as FFLogs cast events — no network,
no fixture file needed) and compares the JSON output to a frozen snapshot.
Complements test_gunbreaker_smoke.py (registration wiring) and
test_gunbreaker_pulls.py (real-pull calibration gates) by locking the actual
wire shape, the same way test_samurai_pipeline.py does for SAM.
...
regression this file exists to catch is GNB ids silently resolving to None
(which blanks the Clipping aspect and the GCD-speed inference).

When the contract intentionally changes, regenerate with:
    UPDATE_SNAPSHOT=1 python tests/test_gunbreaker_pipeline.py

Run from python/:  python tests/test_gunbreaker_pipeline.py
"""
...
def _synthetic_casts() -> list[dict]:
    from jobs.gunbreaker.simulator import simulate_idealized
...
class StubFFLogsClient:
    """Serves the synthetic single-GNB pull; refs flow short-circuited via empty
    ...
    def get_report_summary(self, code: str) -> dict:
        ...
        return {
            "title": "GNB pipeline fixture",
            "startTime": _FIGHT_START_MS,
            "endTime": end_ms,
            "fights": [{
                "id": 1, "name": "Synthetic GNB Fight", "encounterID": 103,
            ...
            "masterData": {
                "actors": [
                    {"id": _SOURCE_ID, "name": "GNB Synthetic Player",
...
def _run_pipeline() -> dict:
    ...
    you = analyze_pull("Gunbreaker", stub, "AbCd1234", 1,
                       ranking_name=None, label="You")
    refs: list = []
    comparisons = _compare_all_aspects("Gunbreaker", you, refs)
    return _build_response("Gunbreaker", you, refs, comparisons)
...
def test_gnb_response_shape(response: dict) -> None:
    """GNB ships only the shared aspects — no MCH-specific ones."""
    ...
    for name in ("Queen", "Wildfire", "Tools"):
        assert name not in states, f"MCH-only aspect {name} leaked into GNB"


def test_gnb_headline_has_efficiency(response: dict) -> None:
...
def test_gnb_ability_meta_resolves_hermetically(response: dict) -> None:
    """Every abilityMeta entry carries a real bundled name + the OGCD_IDS flag —
    the end-to-end proof the GNB block in ability_metadata.BUNDLED is wired.
    (Before the block existed, every GNB id resolved to None under the stub.)"""
    ...
    for aid_str, m in meta.items():
        ...
        if aid in gd.POTENCIES or aid in gd.OGCD_IDS or aid in gd.DEFENSIVE_IDS:
            assert m.get("isOgcd") == (aid in gd.OGCD_IDS), \
                f"{m.get('name')} ({aid}): isOgcd mismatch vs OGCD_IDS"
    # Spot-check two load-bearing ids by name.
    assert meta.get(str(gd.NO_MERCY), {}).get("name") == "No Mercy"
    assert meta.get(str(gd.BURST_STRIKE), {}).get("name") == "Burst Strike"


def test_gnb_clipping_active(response: dict) -> None:
    """The Clipping aspect must actually WALK the casts — it silently skips any
    cast whose ability metadata is None, which blanked the whole aspect before
    GNB was bundled. A blanked walk leaves avg_gcd_potency at its 350.0 default;
    a real walk averages GNB's GCD table (~440+ over the synthetic stream)."""
    ...
    assert finding.get("avgGcdPotency", 350.0) != 350.0, \
        "avgGcdPotency at the metadata-blanked default — GNB ids not resolving"
...
def _snapshot_path() -> Path:
    return SNAPSHOTS_DIR / "gnb_synthetic.snapshot.json"


def test_gnb_snapshot(response: dict) -> None:
    ...
    assert snap_path.exists(), \
        f"no snapshot at {snap_path}; run UPDATE_SNAPSHOT=1 python tests/test_gunbreaker_pipeline.py"
...
def main() -> int:
    ...
    test_gnb_response_shape(resp)
    test_gnb_headline_has_efficiency(resp)
    test_gnb_ability_meta_resolves_hermetically(resp)
    test_gnb_clipping_active(resp)
    test_gnb_snapshot(resp)
    print("all gunbreaker pipeline tests passed")
    return 0
