"""Ninja simulator validation against REAL pulls (no network at test time).

The companion to test_ninja_sim.py (internal invariants). This one validates the
sim against actual human play — real quartile-stratified NIN pulls captured from
FFLogs top rankings (one per quartile per encounter), under tests/fixtures/nin/.
Validating against the sim's own output would be circular; these are real cast
streams, so the tests confirm the sim stays calibrated and never under-rates a
real top parse.

NIN's damage state (Kunai's Bane windows, Kassatsu, Kazematoi, Meisui, Bunshin
mirrors) is all derived from the cast stream itself — no aura/DamageDone capture
is needed, so the fixtures are just casts + targetability + comp.

Regenerate / add encounters with:
    python scripts/add_ninja_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_ninja_pulls.py
"""
...
NIN_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "nin"
...
class MockClient:
    """Serves a real captured NIN pull (casts + targetability + comp). Returns []
    for DamageDone / Buffs (nothing NIN scores needs them). No network."""
        ...
        return {
            ...
            "masterData": {
                "actors": [{
                    ...
                    "subType": "Ninja", "petOwner": None, "gameID": 30,
                }, *other, *npc_actors],
            },
        }
...
def _fixture_names() -> list[str]:
    return [p.stem for p in sorted(NIN_FIXTURES_DIR.glob("*.json"))
            if p.stem != "synthetic"]
...
def _analyze(name: str):
    fix = json.loads((NIN_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Ninja", client, fix["report_code"], fix["fight_id"],
...
_NIN_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no NIN pull fixtures")
...
def test_pull_invariants(name: str) -> None:
    ...
    assert 150 <= pps <= 650, f"{name}: p/sec {pps:.1f} out of band"
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (NIN_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def test_quartile_efficiency_clustered() -> None:
    ...
    quartiles of the elite top-100 execute near-identically (the difference is
    within noise, sometimes inverted — the VPR finding). We assert the quartile
    means are TIGHTLY CLUSTERED + all calibrated — a real ceiling bug would leave
    one quartile far off the others — rather than a (false) topq > botq."""
    ...
    assert all(0.82 <= m <= _EFFICIENCY_TOL for m in means.values()), \
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no NIN pull fixtures — run scripts/add_ninja_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll NIN real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
