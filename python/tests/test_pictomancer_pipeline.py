"""PCT end-to-end pipeline + contract snapshot test.

Drives `analyze_pull` and the sidecar's `_build_response` against a synthetic
PCT pull (the default-sim timeline replayed as FFLogs cast events — no network,
no fixture file needed) and compares the JSON output to a frozen snapshot.
Complements test_pictomancer_sim.py (sim invariants) and
test_pictomancer_pulls.py (real-pull calibration gates) by locking the actual
wire shape, the same way test_bard_pipeline.py does for BRD.
...
regression this guards is PCT ids silently resolving to None (which blanks the
Clipping aspect and the GCD-speed inference).

When the contract intentionally changes, regenerate with:
    UPDATE_SNAPSHOT=1 python tests/test_pictomancer_pipeline.py

Run from python/:  python tests/test_pictomancer_pipeline.py
"""
...
def _synthetic_casts() -> list[dict]:
    from jobs.pictomancer.simulator import simulate_idealized
...
class StubFFLogsClient:
    """Serves the synthetic single-PCT pull; refs flow short-circuited via empty
    ...
    def get_report_summary(self, code: str) -> dict:
        ...
        return {
            "title": "PCT pipeline fixture",
            "startTime": _FIGHT_START_MS,
            "endTime": end_ms,
            "fights": [{
                "id": 1, "name": "Synthetic PCT Fight", "encounterID": 103,
            ...
            "masterData": {
                "actors": [
                    {"id": _SOURCE_ID, "name": "PCT Synthetic Player",
...
def _run_pipeline() -> dict:
    ...
    you = analyze_pull("Pictomancer", stub, "AbCd1234", 1,
                       ranking_name=None, label="You")
    refs: list = []
    comparisons = _compare_all_aspects("Pictomancer", you, refs)
    return _build_response("Pictomancer", you, refs, comparisons)
...
def test_pct_response_shape(response: dict) -> None:
    """PCT ships only the shared aspects — no MCH-specific ones."""
    ...
    for name in ("Queen", "Wildfire", "Tools"):
        assert name not in states, f"MCH-only aspect {name} leaked into PCT"


def test_pct_headline_has_efficiency(response: dict) -> None:
...
def test_pct_ability_meta_resolves_hermetically(response: dict) -> None:
    """Every abilityMeta entry carries a real bundled name + the OGCD_IDS flag —
    the end-to-end proof the PCT block in ability_metadata.BUNDLED is wired."""
    ...
    for aid_str, m in meta.items():
        ...
        if aid in pdta.POTENCIES or aid in pdta.OGCD_IDS or aid in pdta.DEFENSIVE_IDS:
            assert m.get("isOgcd") == (aid in pdta.OGCD_IDS), \
                f"{m.get('name')} ({aid}): isOgcd mismatch vs OGCD_IDS"
    # Spot-check two load-bearing ids by name.
    assert meta.get(str(pdta.STARRY_MUSE), {}).get("name") == "Starry Muse"
    assert meta.get(str(pdta.FIRE_IN_RED), {}).get("name") == "Fire in Red"


def test_pct_clipping_active(response: dict) -> None:
    ...
    the PCT BUNDLED block were missing."""
    ...
    assert finding.get("avgGcdPotency", 350.0) != 350.0, \
        "avgGcdPotency at the metadata-blanked default — PCT ids not resolving"
...
def _snapshot_path() -> Path:
    return SNAPSHOTS_DIR / "pct_synthetic.snapshot.json"


def test_pct_snapshot(response: dict) -> None:
    ...
    assert snap_path.exists(), \
        f"no snapshot at {snap_path}; run UPDATE_SNAPSHOT=1 python tests/test_pictomancer_pipeline.py"
...
def main() -> int:
    ...
    test_pct_response_shape(resp)
    test_pct_headline_has_efficiency(resp)
    test_pct_ability_meta_resolves_hermetically(resp)
    test_pct_clipping_active(resp)
    test_pct_snapshot(resp)
    print("all pictomancer pipeline tests passed")
    return 0
