"""Dragoon simulator validation against REAL pulls (no network at test time).

The companion to test_dragoon_sim.py (internal invariants). This one validates the
sim against actual human play — real quartile-stratified DRG pulls captured from
FFLogs top rankings (one per quartile per encounter), under tests/fixtures/drg/.
Validating against the sim's own output would be circular; these are real cast
streams, so the tests confirm the sim correlates with skill and never under-rates a
real top parse.

DRG's damage self-buffs (Power Surge / Lance Charge / Life of the Dragon) ride the
player's own casts, so the captured cast stream is sufficient — no per-pull scalar.
The captured fixtures carry no DamageDone stream, so the PositionalAspect falls back
to assume-hit (no positional penalty), which is accurate for top pulls.

Regenerate / add encounters with:
    python scripts/add_dragoon_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_dragoon_pulls.py
"""
...
DRG_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "drg"
_BUCKETS = ("topq", "q2", "q3", "botq")
# Per-job efficiency tolerance. The ceiling is a true upper bound, so NO real pull
# may exceed 100% (delivered <= idealized by construction). DRG's ceiling runs at the
# gear-true 2.50 GCD; this is a genuine correctness guard, not a fudge factor.
_EFFICIENCY_TOL = 1.005


class MockClient:
    """Serves a real captured DRG pull (casts + targetability + comp). Returns [] for
    DamageDone / Buffs so positionals fall back to assume-hit. No network."""
...
def _fixture_names() -> list[str]:
    return [p.stem for p in sorted(DRG_FIXTURES_DIR.glob("*.json"))
            if p.stem != "synthetic"]
...
def _analyze(name: str):
    fix = json.loads((DRG_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Dragoon", client, fix["report_code"], fix["fight_id"],
...
_DRG_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring", "Positionals", "LifeSurge"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no DRG pull fixtures")
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (DRG_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def test_quartile_efficiency_clustered() -> None:
    ...
    band. The metric is crit/gear-NEUTRAL (it scores potency execution, not rDPS), so
    the FFLogs rDPS ranking does NOT predict it — the elite top-100 execute
    near-identically. A real ceiling bug would leave one quartile far off the others;
    we assert tightly-clustered + all-calibrated rather than a (false) topq > botq.
    See the efficiency-is-crit-neutral-potency memory."""
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no DRG pull fixtures — run scripts/add_dragoon_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll DRG real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
