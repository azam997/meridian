"""Dark Knight simulator validation against REAL pulls (no network at test time).

The companion to test_darkknight_sim.py (internal invariants). This one validates
the sim against actual human play — real quartile-stratified DRK pulls captured
from FFLogs top rankings (one per quartile per encounter), under
tests/fixtures/drk/. Validating against the sim's own output would be circular;
these are real cast streams, so the tests confirm the sim correlates with skill
and never under-rates a real top parse.

DRK is RNG-free — the ceiling is pure (duration, downtime) data plus the
per-player GCD: the Esteem pet contribution is folded onto the Living Shadow
cast id, Darkside is derived from the player's own Edge/Flood casts, and
TBN/Dark Arts is MP-net-neutral (see jobs/darkknight/data.py), so no per-pull
budget context is needed.

Regenerate / add encounters with:
    python scripts/add_darkknight_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_darkknight_pulls.py
"""
...
DRK_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "drk"
_BUCKETS = ("topq", "q2", "q3", "botq")
# Per-job efficiency tolerance. The ceiling is a true upper bound, so NO real
# pull may exceed 100% (delivered <= idealized by construction) — a genuine
# correctness guard, not a fudge factor.
_EFFICIENCY_TOL = 1.0


class MockClient:
    """Serves a real captured DRK pull (casts + targetability + comp). Returns []
    for Buffs / DamageDone (no observed tincture/raid windows — the strict lens
    under test is buff-agnostic anyway). No network, no pet actors — the Esteem
    fold rides the Living Shadow cast id."""
...
def _fixture_names() -> list[str]:
    if not DRK_FIXTURES_DIR.exists():
        return []
    return [p.stem for p in sorted(DRK_FIXTURES_DIR.glob("*.json"))
            if p.stem != "synthetic"]
...
def _analyze(name: str):
    fix = json.loads((DRK_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Dark Knight", client, fix["report_code"], fix["fight_id"],
...
_DRK_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no DRK pull fixtures")
@pytest.mark.parametrize("name", _FIXTURE_NAMES)
def test_pull_invariants(name: str) -> None:
    """Per real pull: pipeline runs, every aspect present, delivered potency in
    a sane p/sec band, and idealized >= delivered within the per-job tolerance."""
    mr, fix = _analyze(name)
    for aname in _DRK_ASPECTS:
    ...
    assert 180 <= pps <= 420, f"{name}: p/sec {pps:.1f} out of band"
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (DRK_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def test_quartile_efficiency_clustered() -> None:
    """Efficiency clusters tightly across the rDPS-ranked quartiles — and all
    stay in a sane band. The metric is crit/gear-NEUTRAL (it scores potency
    execution, not rDPS), so the FFLogs rDPS ranking does NOT predict it: the
    top and bottom quartiles of the elite top-100 execute near-identically. We
    assert the quartile means are TIGHTLY CLUSTERED + all calibrated — a real
    ceiling bug would leave one quartile far off the others — rather than a
    (false) topq > botq (the VPR finding)."""
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no DRK pull fixtures — run scripts/add_darkknight_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll DRK real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
