"""Viper simulator validation against REAL pulls (no network at test time).

The companion to test_viper_sim.py (internal invariants). This one validates the
sim against actual human play — real quartile-stratified VPR pulls captured from
FFLogs top rankings (one per quartile per encounter), under tests/fixtures/vpr/.
...
Hunter's Instinct coverage: the captured fixtures carry no DamageDone stream, so
`measured_hunters_instinct_intervals` falls back to FULL coverage; the x1.10 then
cancels in delivered/idealized (accurate for top pulls, which hold it ~100%).

Regenerate / add encounters with:
    python scripts/add_viper_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_viper_pulls.py
"""
...
VPR_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "vpr"
_BUCKETS = ("topq", "q2", "q3", "botq")
# Per-job efficiency tolerance. The ceiling is a true upper bound, so NO real pull
# may exceed 100% (delivered <= idealized by construction). VPR's ceiling runs at the
# weave-bound sustained cadence (live top parses ~96-99%), so this is a genuine
# correctness guard, not a fudge factor.
_EFFICIENCY_TOL = 1.0


class MockClient:
    """Serves a real captured VPR pull (casts + targetability + comp). Returns []
    for DamageDone / Buffs so Hunter's Instinct falls back to full coverage. No
    network."""
        ...
        return {
            ...
            "masterData": {
                "actors": [{
                    ...
                    "subType": "Viper", "petOwner": None, "gameID": 41,
                }, *other, *npc_actors],
            },
        }
...
def _fixture_names() -> list[str]:
    return [p.stem for p in sorted(VPR_FIXTURES_DIR.glob("*.json"))
            if p.stem != "synthetic"]
...
def _analyze(name: str):
    fix = json.loads((VPR_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Viper", client, fix["report_code"], fix["fight_id"],
...
_VPR_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no VPR pull fixtures")
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (VPR_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def test_quartile_efficiency_clustered() -> None:
    ...
    difference is within noise, sometimes inverted). That gear-neutrality is the
    design intent (see the efficiency-is-crit-neutral-potency memory), so we assert
    the quartile means are TIGHTLY CLUSTERED + all calibrated — a real ceiling bug
    would leave one quartile far off the others — rather than a (false) topq > botq."""
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no VPR pull fixtures — run scripts/add_viper_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll VPR real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
