"""BRD end-to-end pipeline + contract snapshot test.

Drives `analyze_pull` and the sidecar's `_build_response` against a synthetic
BRD pull (the default-sim timeline replayed as FFLogs cast events — no network,
no fixture file needed) and compares the JSON output to a frozen snapshot.
Complements test_bard_sim.py (sim invariants) and test_bard_pulls.py (real-pull
calibration gates) by locking the actual wire shape, the same way
test_gunbreaker_pipeline.py does for GNB.
...
regression this guards is BRD ids silently resolving to None (which blanks the
Clipping aspect and the GCD-speed inference).

When the contract intentionally changes, regenerate with:
    UPDATE_SNAPSHOT=1 python tests/test_bard_pipeline.py

Run from python/:  python tests/test_bard_pipeline.py
"""
...
def _synthetic_casts() -> list[dict]:
    from jobs.bard.simulator import simulate_idealized
...
class StubFFLogsClient:
    """Serves the synthetic single-BRD pull; refs flow short-circuited via empty
    ...
    def get_report_summary(self, code: str) -> dict:
        ...
        return {
            "title": "BRD pipeline fixture",
            "startTime": _FIGHT_START_MS,
            "endTime": end_ms,
            "fights": [{
                "id": 1, "name": "Synthetic BRD Fight", "encounterID": 103,
            ...
            "masterData": {
                "actors": [
                    {"id": _SOURCE_ID, "name": "BRD Synthetic Player",
...
def _run_pipeline() -> dict:
    ...
    you = analyze_pull("Bard", stub, "AbCd1234", 1,
                       ranking_name=None, label="You")
    refs: list = []
    comparisons = _compare_all_aspects("Bard", you, refs)
    return _build_response("Bard", you, refs, comparisons)
...
def test_brd_response_shape(response: dict) -> None:
    """BRD ships only the shared aspects — no MCH-specific ones."""
    ...
    for name in ("Queen", "Wildfire", "Tools"):
        assert name not in states, f"MCH-only aspect {name} leaked into BRD"


def test_brd_headline_has_efficiency(response: dict) -> None:
...
def test_brd_ability_meta_resolves_hermetically(response: dict) -> None:
    """Every abilityMeta entry carries a real bundled name + the OGCD_IDS flag —
    the end-to-end proof the BRD block in ability_metadata.BUNDLED is wired."""
    ...
    for aid_str, m in meta.items():
        ...
        if aid in bd.POTENCIES or aid in bd.OGCD_IDS or aid in bd.DEFENSIVE_IDS:
            assert m.get("isOgcd") == (aid in bd.OGCD_IDS), \
                f"{m.get('name')} ({aid}): isOgcd mismatch vs OGCD_IDS"
    # Spot-check two load-bearing ids by name.
    assert meta.get(str(bd.RADIANT_FINALE), {}).get("name") == "Radiant Finale"
    assert meta.get(str(bd.BURST_SHOT), {}).get("name") == "Burst Shot"


def test_brd_clipping_active(response: dict) -> None:
    ...
    the BRD BUNDLED block were missing."""
    ...
    assert finding.get("avgGcdPotency", 350.0) != 350.0, \
        "avgGcdPotency at the metadata-blanked default — BRD ids not resolving"


def test_brd_budgets_on_wire(response: dict) -> None:
    """The measured RNG budgets ride the Scoring state onto the wire (camelized),
    so the UI / sidecar recompute paths can spend the same counts."""
    scoring = response.get("aspectStates", {}).get("Scoring", {})
    for key in ("refulgentBudget", "ppBudget", "apexBudget", "blastBudget",
                "heartbreakBudget"):
        assert key in scoring, f"Scoring state missing {key}"
...
def _snapshot_path() -> Path:
    return SNAPSHOTS_DIR / "brd_synthetic.snapshot.json"


def test_brd_snapshot(response: dict) -> None:
    ...
    assert snap_path.exists(), \
        f"no snapshot at {snap_path}; run UPDATE_SNAPSHOT=1 python tests/test_bard_pipeline.py"
...
def main() -> int:
    ...
    test_brd_response_shape(resp)
    test_brd_headline_has_efficiency(resp)
    test_brd_ability_meta_resolves_hermetically(resp)
    test_brd_clipping_active(resp)
    test_brd_budgets_on_wire(resp)
    test_brd_snapshot(resp)
    print("all bard pipeline tests passed")
    return 0
