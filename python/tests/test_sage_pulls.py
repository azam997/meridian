"""Sage simulator validation against REAL pulls (no network at test time).

The companion to test_sage_sim.py (internal invariants). This one validates the sim
against actual human play — real quartile-stratified SGE pulls captured from FFLogs
top rankings, under tests/fixtures/sge/. Validating against the sim's own output
would be circular; these are real cast streams, so the tests confirm the sim never
under-rates a real top parse (efficiency <= 100%).

Healer note: a real SGE spends GCDs healing (Eukrasian Prognosis / Diagnosis /
Prognosis …) and casts Pneuma / Toxikon for movement + free heals, which is genuine
damage-efficiency loss vs the *unlocked* ceiling — so even top healers sit well below
100% here (the mit-plan lock pays that tax in the product flow). Efficiency is
gear-neutral, so the metric is NOT rDPS-ordered across quartiles; the real signal is
TIGHT clustering, not topq > botq (the VPR finding).

Regenerate / add encounters with:
    python scripts/add_sage_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_sage_pulls.py
"""
...
SGE_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "sge"
...
class MockClient:
    """Serves a real captured SGE pull (casts + targetability + comp). No buff
        ...
        return {
            ...
            "masterData": {
                "actors": [{
                    ...
                    "subType": "Sage", "petOwner": None, "gameID": 40,
                }, *other, *npc_actors],
            },
        }
...
def _fixture_names() -> list[str]:
    return [p.stem for p in sorted(SGE_FIXTURES_DIR.glob("*.json"))
            if p.stem != "synthetic"]
...
def _analyze(name: str):
    fix = json.loads((SGE_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Sage", client, fix["report_code"], fix["fight_id"],
...
_SGE_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no SGE pull fixtures")
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (SGE_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no SGE pull fixtures — run scripts/add_sage_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll SGE real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
