"""Astrologian simulator validation against REAL pulls (no network at test time).

The companion to test_astrologian_sim.py (internal invariants). This one
validates the sim against actual human play — real quartile-stratified AST pulls
captured from FFLogs top rankings, under tests/fixtures/ast/. Validating against
the sim's own output would be circular; these are real cast streams, so the tests
confirm the sim never under-rates a real top parse (efficiency <= 100%).

Healer note: a real AST spends GCDs healing (Benefic / Helios / Aspected …),
which is genuine damage-efficiency loss vs the *unlocked* ceiling — so even top
healers sit well below 100% here (the mit-plan lock pays that tax in the product
flow). Efficiency is gear-neutral, so the metric is NOT rDPS-ordered across
quartiles; the real signal is TIGHT clustering, not topq > botq (the VPR finding).

Regenerate / add encounters with:
    python scripts/add_astrologian_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Run from python/:  python tests/test_astrologian_pulls.py
"""
...
AST_FIXTURES_DIR = Path(__file__).parent / "fixtures" / "ast"
...
class MockClient:
    """Serves a real captured AST pull (casts + targetability + comp). No buff
    stream in the fixtures -> no observed raid buffs / tincture windows (the
    strict scenario is the assertion target anyway)."""
        ...
        return {
            ...
            "masterData": {
                "actors": [{
                    ...
                    "subType": "Astrologian", "petOwner": None, "gameID": 33,
                }, *other, *npc_actors],
            },
        }
...
def _fixture_names() -> list[str]:
    return [p.stem for p in sorted(AST_FIXTURES_DIR.glob("*.json"))
            if p.stem != "synthetic"]
...
def _analyze(name: str):
    fix = json.loads((AST_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))
    client = MockClient(fix)
    mr = analyze_pull("Astrologian", client, fix["report_code"], fix["fight_id"],
...
_AST_ASPECTS = ["Abilities", "Drift", "Clipping", "Overcap", "Opener",
                "Alignment", "BuffDrift", "Scoring"]


@pytest.mark.skipif(not _FIXTURE_NAMES, reason="no AST pull fixtures")
...
def test_sim_monotonicity(name: str) -> None:
    ...
    dur = json.loads(
        (AST_FIXTURES_DIR / f"{name}.json").read_text(encoding="utf-8"))["duration_s"]
...
def test_quartile_clustering() -> None:
    """Efficiency is crit/gear-neutral, so the metric is NOT rDPS-ordered — the
    real signal is TIGHT clustering across quartiles (a wide spread is the bug
    signal). Assert every real pull is a valid parse (<= tol) and the quartile
    means sit within a modest band."""
    effs: list[float] = []
    ...
    # Clustering: the whole sample spans less than ~20pp (healer heal-tax varies
    # by fight, but a single encounter's real pulls should cluster tightly).
...
def main() -> int:
    if not _FIXTURE_NAMES:
        print("no AST pull fixtures — run scripts/add_astrologian_fixtures.py")
        return 0
    for name in _FIXTURE_NAMES:
    ...
    print(f"\nAll AST real-pull tests passed ({len(_FIXTURE_NAMES)} fixtures)")
    return 0
