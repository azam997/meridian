...
# ninjutsu). In the SIMULATOR the entry does double duty deliberately: the manual
# `state.charges[TEN] -= 1` is the only spend path (the mudra branch returns
# before apply_cooldown) while the entry itself feeds the engine's generic
# multi-charge regen, including through downtime, which is the point. On the
# DELIVERED side, Chi/Jin-opened ninjutsu (Katon opens Chi!) drain the same pool
# — `charge_sharing` below maps them onto TEN so the drift detector's charge
# accounting sees every mudra spend, not just Ten-opened ones. Bhavacakra /
...
# A banked mudra charge cashes out as roughly a Raiton line's NET value over the
# filler GCD it displaces (mirrors the simulator's `_PRUNE_CHARGE_P` beam
# credit). Used only by the deep-advice charge-economy ledger
# (jobs/ninja/advice.py); enabler/improvement pricing still comes from the
# sim's marginal values.
MUDRA_CHARGE_VALUE_P: float = 400.0
...
JOB_DATA: JobData = JobData(
    ...
    cdr_rules=(),
    # Chi/Jin first-mudra presses drain TEN's shared 2-charge pool (an AoE
    # Ninja's Katon opens Chi→Ten, so its charge spends log under CHI).
    charge_sharing={CHI: TEN, JIN: TEN},
