"""Pull a quartile-stratified sample of REAL Sage pulls for an encounter and save
them under tests/fixtures/sge/ for the real-data simulator tests.

The SGE analogue of add_scholar_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_sage_pulls.py checks the sim against actual human play. Handles
both Savage and ultimate encounters (difficulty from the catalog).

Run from python/:
    python scripts/add_sage_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_sage_fixtures.py 103 tyrant 1
    python scripts/add_sage_fixtures.py 104 m12sp1 1
"""
...
JOB = "Sage"
FFLOGS_SUBTYPE = "Sage"                                # FFLogs reports it spaceless
SGE_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "sge")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    sge = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in sge if a["name"].lower() == (name or "").lower()]
    pick = by_name or sge
...
def main() -> int:
    ...
    print(f"Fetching Sage rankings for encounter {encounter_id} "
    ...
    SGE_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no SGE actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            # Stamp the encounter id + difficulty so the pulls-test mock builds the
            # right report summary (ultimate fixtures need difficulty 100).
            fixture["encounter_id"] = encounter_id
            fixture["difficulty"] = difficulty
            (SGE_FIXTURES_DIR / f"{label}.json").write_text(
