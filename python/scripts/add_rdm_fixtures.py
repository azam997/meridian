"""Pull a quartile-stratified sample of REAL Red Mage pulls for an encounter and
save them under tests/fixtures/rdm/ for the real-data simulator tests.

This is the RDM analogue of add_encounter_fixtures.py (MCH). Validating the sim
against the player's *own* synthetic output is circular self-validation; these
fixtures are real top-rankings pulls across quartiles so the tests check the sim
against actual human play (and that efficiency correlates with skill).

Run from python/:
    python scripts/add_rdm_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_rdm_fixtures.py 101 vampfatale 1
"""
...
JOB = "Red Mage"
FFLOGS_SUBTYPE = "RedMage"                              # FFLogs reports it spaceless
RDM_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "rdm")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    rdm = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in rdm if a["name"].lower() == (name or "").lower()]
    pick = by_name or rdm
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching RDM rankings for encounter {encounter_id} ...")
    ...
    RDM_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no RDM actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (RDM_FIXTURES_DIR / f"{label}.json").write_text(
