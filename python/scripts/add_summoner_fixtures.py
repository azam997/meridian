"""Pull a quartile-stratified sample of REAL Summoner pulls for an encounter
and save them under tests/fixtures/smn/ for the real-data simulator tests.

The SMN analogue of add_pictomancer_fixtures.py. Validating the sim against its
own synthetic output is circular; these fixtures are real top-rankings pulls
across quartiles so test_summoner_pulls.py checks the sim against actual human
play. SMN is RNG-free, so there's no budget capture — the cast stream itself is
everything the ceiling needs (the pet contributions are folded onto the
player's own cast ids).

Run from python/:
    python scripts/add_summoner_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_summoner_fixtures.py 103 tyrant 1
"""
...
SMN_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "smn")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    smn = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in smn if a["name"].lower() == (name or "").lower()]
    pick = by_name or smn
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Summoner rankings for encounter {encounter_id} ...")
    ...
    SMN_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no SMN actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (SMN_FIXTURES_DIR / f"{label}.json").write_text(
