"""Pull a quartile-stratified sample of REAL White Mage pulls for an encounter
and save them under tests/fixtures/whm/ for the real-data simulator tests.

The WHM analogue of add_samurai_fixtures.py. Validating the sim against its
own synthetic output is circular; these fixtures are real top-rankings pulls
across quartiles so test_whitemage_pulls.py checks the sim against actual
human play (and that efficiency correlates with skill).

Run from python/:
    python scripts/add_whitemage_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_whitemage_fixtures.py 103 tyrant 1
"""
...
JOB = "White Mage"
FFLOGS_SUBTYPE = "WhiteMage"                          # FFLogs reports it spaceless
WHM_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "whm")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    whm = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in whm if a["name"].lower() == (name or "").lower()]
    pick = by_name or whm
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching White Mage rankings for encounter {encounter_id} ...")
    ...
    WHM_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no WHM actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (WHM_FIXTURES_DIR / f"{label}.json").write_text(
