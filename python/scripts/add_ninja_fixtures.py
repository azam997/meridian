"""Pull a quartile-stratified sample of REAL Ninja pulls for an encounter and save
them under tests/fixtures/nin/ for the real-data simulator tests.

The NIN analogue of add_viper_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_ninja_pulls.py checks the sim against actual human play (and
that efficiency stays calibrated). NIN's self-buffs (Kunai's Bane / Kassatsu /
Meisui / Bunshin / Kazematoi) are all derived from the cast stream itself, so the
...
Example:
    python scripts/add_ninja_fixtures.py 104 m12sp1 1
"""
...
JOB = "Ninja"
FFLOGS_SUBTYPE = "Ninja"
NIN_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "nin")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    nin = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in nin if a["name"].lower() == (name or "").lower()]
    pick = by_name or nin
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Ninja rankings for encounter {encounter_id} ...")
    ...
    NIN_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no NIN actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (NIN_FIXTURES_DIR / f"{label}.json").write_text(
