"""Pull a quartile-stratified sample of REAL Dancer pulls for an encounter and
save them under tests/fixtures/dnc/ for the real-data simulator tests.

Validating the sim against its own synthetic output is circular; these fixtures
are real top-rankings pulls across quartiles so test_dancer_pulls.py checks the
sim against actual human play (and that efficiency correlates with skill). DNC
needs no extra captured input beyond the cast stream — its proc/feather/esprit
budgets are measured straight from the player's casts.

Run from python/:
    python scripts/add_dancer_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_dancer_fixtures.py 103 tyrant 1
"""
...
JOB = "Dancer"
FFLOGS_SUBTYPE = "Dancer"
DNC_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "dnc")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    dnc = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in dnc if a["name"].lower() == (name or "").lower()]
    pick = by_name or dnc
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Dancer rankings for encounter {encounter_id} ...")
    ...
    DNC_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no DNC actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (DNC_FIXTURES_DIR / f"{label}.json").write_text(
