"""Pull a quartile-stratified sample of REAL Dragoon pulls for an encounter and
save them under tests/fixtures/drg/ for the real-data simulator tests.

The DRG analogue of add_samurai_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_dragoon_pulls.py checks the sim against actual human play (and
that efficiency correlates with skill). DRG has no measured per-pull scalar (its
self-buffs ride the player's own casts), so the capture is just the cast /
targetability / DamageDone streams.

Run from python/:
    python scripts/add_dragoon_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_dragoon_fixtures.py 103 m11s 1
"""
...
JOB = "Dragoon"
FFLOGS_SUBTYPE = "Dragoon"
DRG_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "drg")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    drg = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in drg if a["name"].lower() == (name or "").lower()]
    pick = by_name or drg
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Dragoon rankings for encounter {encounter_id} ...")
    ...
    DRG_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no DRG actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (DRG_FIXTURES_DIR / f"{label}.json").write_text(
