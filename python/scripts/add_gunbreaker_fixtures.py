"""Pull a quartile-stratified sample of REAL Gunbreaker pulls for an encounter and
save them under tests/fixtures/gnb/ for the real-data simulator tests.

The GNB analogue of add_dragoon_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_gunbreaker_pulls.py checks the sim against actual human play (and
that efficiency clusters under the 100.5% gate). GNB has no measured per-pull scalar
(its No Mercy self-buff rides the player's own casts), so the capture is just the cast
...
Example:
    python scripts/add_gunbreaker_fixtures.py 103 m11s 1
"""
...
JOB = "Gunbreaker"
FFLOGS_SUBTYPE = "Gunbreaker"
GNB_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "gnb")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    gnb = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in gnb if a["name"].lower() == (name or "").lower()]
    pick = by_name or gnb
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Gunbreaker rankings for encounter {encounter_id} ...")
    ...
    GNB_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no GNB actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (GNB_FIXTURES_DIR / f"{label}.json").write_text(
