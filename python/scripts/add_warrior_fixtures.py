"""Pull a quartile-stratified sample of REAL Warrior pulls for an encounter and
save them under tests/fixtures/war/ for the real-data simulator tests.

The WAR analogue of add_paladin_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_warrior_pulls.py checks the sim against actual human play (and
...
Example:
    python scripts/add_warrior_fixtures.py 102 m10s 1
"""
...
JOB = "Warrior"
FFLOGS_SUBTYPE = "Warrior"
WAR_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "war")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    war = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in war if a["name"].lower() == (name or "").lower()]
    pick = by_name or war
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Warrior rankings for encounter {encounter_id} ...")
    ...
    WAR_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no WAR actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (WAR_FIXTURES_DIR / f"{label}.json").write_text(
