"""Pull a quartile-stratified sample of REAL Scholar pulls for an encounter and
save them under tests/fixtures/sch/ for the real-data simulator tests.

The SCH analogue of add_astrologian_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_scholar_pulls.py checks the sim against actual human play. Handles
...
Example:
    python scripts/add_scholar_fixtures.py 103 tyrant 1
    python scripts/add_scholar_fixtures.py 1085 dancingmad 1
"""
...
JOB = "Scholar"
FFLOGS_SUBTYPE = "Scholar"                             # FFLogs reports it spaceless
SCH_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "sch")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    sch = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in sch if a["name"].lower() == (name or "").lower()]
    pick = by_name or sch
...
def main() -> int:
    ...
    print(f"Fetching Scholar rankings for encounter {encounter_id} "
    ...
    SCH_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no SCH actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            (SCH_FIXTURES_DIR / f"{label}.json").write_text(
