"""Pull a quartile-stratified sample of REAL Paladin pulls for an encounter and
save them under tests/fixtures/pld/ for the real-data simulator tests.

The PLD analogue of add_reaper_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_paladin_pulls.py checks the sim against actual human play (and
that efficiency correlates with skill, and never rates a real top parse > 100%).

Run from python/ (needs FFLogs credentials in ~/.fflogs_efficiency_analyzer/config.json):
    python scripts/add_paladin_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_paladin_fixtures.py 102 m10s 1
"""
...
JOB = "Paladin"
FFLOGS_SUBTYPE = "Paladin"
PLD_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "pld")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    pld = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in pld if a["name"].lower() == (name or "").lower()]
    pick = by_name or pld
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Paladin rankings for encounter {encounter_id} ...")
    ...
    PLD_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no PLD actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (PLD_FIXTURES_DIR / f"{label}.json").write_text(
