"""Pull a quartile-stratified sample of REAL Dark Knight pulls for an encounter
and save them under tests/fixtures/drk/ for the real-data simulator tests.

The DRK analogue of add_summoner_fixtures.py. Validating the sim against its
own synthetic output is circular; these fixtures are real top-rankings pulls
across quartiles so test_darkknight_pulls.py checks the sim against actual
human play. DRK is RNG-free, so there's no budget capture — the cast stream
itself is everything the ceiling needs (the Esteem fold rides the Living Shadow
cast id, Darkside is derived from the player's own Edge/Flood casts, and
TBN/Dark Arts is MP-net-neutral).

Run from python/:
    python scripts/add_darkknight_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_darkknight_fixtures.py 103 tyrant 1
"""
...
DRK_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "drk")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    drk = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in drk if a["name"].lower() == (name or "").lower()]
    pick = by_name or drk
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Dark Knight rankings for encounter {encounter_id} ...")
    blob = client.get_rankings(
        encounter_id, class_name=FFLOGS_SUBTYPE, spec_name=FFLOGS_SUBTYPE,
    ...
    DRK_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no DRK actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (DRK_FIXTURES_DIR / f"{label}.json").write_text(
