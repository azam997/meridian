"""One-off: dump authoritative Ninja action ids + buff-status ids from real
top-Ninja pulls' masterData.abilities, so data.py can be written against ground
truth. Also prints the opening cast sequence (for the canonical opener + the
pre-pull Suiton timing), the GCD cadence between consecutive normal weaponskills
(the Huton-hasted GCD constant), and the mudra->ninjutsu spacing. Run from python/:
    python scripts/probe_ninja_ids.py [--enc 103] [--top 3]
"""
...
def _dump_one(client, r) -> None:
    ...
    if not actors:
        print("  (no Ninja actor in this fight)")
        return
    nin = next((a for a in actors if a["name"].lower() == (r.get("name") or "").lower()),
               actors[0])
    aid = nin["id"]
    ...
    # Buff durations (first apply->remove pair per status) — Shadow Walker,
    # Bunshin, Kassatsu, Meisui, Raiju Ready, Medicated.
    ...
    # GCD cadence: consecutive-cast gaps histogram (rounded to 0.05s) for the most
    # common spacings — separates the hasted normal GCD (~2.12), the 1.5s ninjutsu,
    # the 0.5s mudras, and weave-stretched gaps.
    ...
    for cid, evs in sorted(by_ab.items(), key=lambda kv: -len(kv[1])):
        ...
        print(f"    {cid:7d}  {abil.get(cid, '?'):26s} x{len(evs):3d} "
              f"mean={mean:9.0f}  bp={dict(bps.most_common(4))}")
