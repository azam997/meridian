"""Pull a quartile-stratified sample of REAL Reaper pulls for an encounter and
save them under tests/fixtures/rpr/ for the real-data simulator tests.

The RPR analogue of add_rdm_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_reaper_pulls.py checks the sim against actual human play (and
...
Example:
    python scripts/add_reaper_fixtures.py 102 m10s 1
"""
...
JOB = "Reaper"
FFLOGS_SUBTYPE = "Reaper"                              # FFLogs reports it spaceless
RPR_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "rpr")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    rpr = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in rpr if a["name"].lower() == (name or "").lower()]
    pick = by_name or rpr
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Reaper rankings for encounter {encounter_id} ...")
    ...
    RPR_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no RPR actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (RPR_FIXTURES_DIR / f"{label}.json").write_text(
