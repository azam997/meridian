"""Pull a quartile-stratified sample of REAL Bard pulls for an encounter and
save them under tests/fixtures/brd/ for the real-data simulator tests.

The BRD analogue of add_samurai_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_bard_pulls.py checks the sim against actual human play. BRD's
per-pull sim_context (the RNG-resource budgets) is measured from the cast
stream itself, which fixtures already carry — no extra capture needed.

Run from python/:
    python scripts/add_bard_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_bard_fixtures.py 103 tyrant 1
"""
...
JOB = "Bard"
FFLOGS_SUBTYPE = "Bard"
BRD_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "brd")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    brd = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in brd if a["name"].lower() == (name or "").lower()]
    pick = by_name or brd
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Bard rankings for encounter {encounter_id} ...")
    ...
    BRD_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no BRD actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (BRD_FIXTURES_DIR / f"{label}.json").write_text(
