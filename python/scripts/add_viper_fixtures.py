"""Pull a quartile-stratified sample of REAL Viper pulls for an encounter and save
them under tests/fixtures/vpr/ for the real-data simulator tests.

The VPR analogue of add_samurai_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_viper_pulls.py checks the sim against actual human play (and that
efficiency correlates with skill). Viper has no measured per-pull scalar (Hunter's
Instinct falls back to full coverage on the captured no-DamageDone stream), so the
capture is just the cast / targetability streams.

Run from python/:
    python scripts/add_viper_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_viper_fixtures.py 105 m12sp2 1
"""
...
JOB = "Viper"
FFLOGS_SUBTYPE = "Viper"                              # FFLogs reports it spaceless
VPR_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "vpr")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    vpr = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in vpr if a["name"].lower() == (name or "").lower()]
    pick = by_name or vpr
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Viper rankings for encounter {encounter_id} ...")
    ...
    VPR_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no VPR actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (VPR_FIXTURES_DIR / f"{label}.json").write_text(
