"""Pull a quartile-stratified sample of REAL Monk pulls for an encounter and save
them under tests/fixtures/mnk/ for the real-data simulator tests.

The MNK analogue of add_ninja_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_monk_pulls.py checks the sim against actual human play (and
that efficiency stays calibrated). MNK's self-buffs (Riddle of Fire / forms /
Fury / Perfect Balance) are all derived from the cast stream itself, and the
chakra budget is the fixture's own The Forbidden Chakra count — so the capture
is just the cast / targetability streams.

Run from python/:
    python scripts/add_monk_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_monk_fixtures.py 104 m12sp1 1
"""
...
JOB = "Monk"
FFLOGS_SUBTYPE = "Monk"
MNK_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "mnk")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    mnk = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in mnk if a["name"].lower() == (name or "").lower()]
    pick = by_name or mnk
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Monk rankings for encounter {encounter_id} ...")
    ...
    MNK_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no MNK actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (MNK_FIXTURES_DIR / f"{label}.json").write_text(
