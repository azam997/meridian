"""Pull a quartile-stratified sample of REAL Pictomancer pulls for an encounter
and save them under tests/fixtures/pct/ for the real-data simulator tests.

The PCT analogue of add_bard_fixtures.py. Validating the sim against its own
synthetic output is circular; these fixtures are real top-rankings pulls across
quartiles so test_pictomancer_pulls.py checks the sim against actual human
play. PCT is RNG-free, so there's no budget capture — the cast stream itself is
everything the ceiling needs.

Run from python/:
    python scripts/add_pictomancer_fixtures.py <encounter_id> <prefix> [n_per_bucket]

Example:
    python scripts/add_pictomancer_fixtures.py 103 tyrant 1
"""
...
PCT_FIXTURES_DIR = (Path(__file__).resolve().parent.parent
                    / "tests" / "fixtures" / "pct")
...
def _resolve_source_id(client, code: str, fight_id: int, name: str) -> int | None:
    ...
    pct = [a for a in actors
           if a["type"] == "Player" and a.get("subType") == FFLOGS_SUBTYPE
           and a["id"] in friendly]
    by_name = [a for a in pct if a["name"].lower() == (name or "").lower()]
    pick = by_name or pct
...
def main() -> int:
    ...
    client = _client()
    print(f"Fetching Pictomancer rankings for encounter {encounter_id} ...")
    ...
    PCT_FIXTURES_DIR.mkdir(parents=True, exist_ok=True)
    saved = 0
    for bucket_name, bucket in buckets.items():
        for j in range(min(n_per_bucket, len(bucket))):
            ...
            if source_id is None:
                print(f"  WARN: no PCT actor for {bucket_name}_{j+1} ({code}/{fight_id})")
                continue
            ...
            if not fixture:
                continue
            (PCT_FIXTURES_DIR / f"{label}.json").write_text(
