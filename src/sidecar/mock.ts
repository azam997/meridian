...
// --- Pulls screen fixtures ---------------------------------------------------
// Spans several date groups (two "today" rows so the Today group renders in
// dev), three jobs (incl. a healer for the "Analyze vs plan" path), both
// categories, and both outcomes. Kill rows reuse MOCK_PULLS' codes so the mock
// runAnalysis works on them; wipe rows reuse the prog codes so the prog
// headline variant triggers.
const MOCK_PULL_ROWS: PullRow[] = [
  {
    reportCode: 'RzHAyvkP2w7Y4T8F', fightId: 14, encounterId: 101,
    job: 'Machinist', kill: true,
    startTimeMs: Date.now() - 2 * 3600_000, durationS: 520,
    parsePct: 66.6, dps: 36600,
    fightPercentage: null, bossPercentage: null, lastPhase: null,
  },
  {
    reportCode: 'ProgMockRep1AaBb', fightId: 31, encounterId: 1085,
    job: 'Machinist', kill: false,
    startTimeMs: Date.now() - 5 * 3600_000, durationS: 412,
    parsePct: null, dps: null,
    fightPercentage: 41.3, bossPercentage: 55.2, lastPhase: 3,
  },
  {
    reportCode: 'ProgMockRep1AaBb', fightId: 28, encounterId: 1085,
    job: 'White Mage', kill: false,
    startTimeMs: Date.parse('2026-08-07T23:52:00'), durationS: 233,
    parsePct: null, dps: null,
    fightPercentage: 68, bossPercentage: 12.4, lastPhase: 2,
  },
  {
    reportCode: 'wJk8mPxz3vN5Q1R7', fightId: 9, encounterId: 104,
    job: 'Machinist', kill: true,
    startTimeMs: Date.parse('2026-08-07T22:11:00'), durationS: 510,
    parsePct: 100, dps: 38100,
    fightPercentage: null, bossPercentage: null, lastPhase: null,
  },
  {
    reportCode: 'aBcDeFg2HiJkL3Mn', fightId: 22, encounterId: 103,
    job: 'White Mage', kill: true,
    startTimeMs: Date.parse('2026-08-05T21:02:00'), durationS: 627,
    parsePct: 17.2, dps: 12100,
    fightPercentage: null, bossPercentage: null, lastPhase: null,
  },
  {
    reportCode: 'RzHAyvkP2w7Y4T8F', fightId: 8, encounterId: 101,
    job: 'Samurai', kill: true,
    startTimeMs: Date.parse('2026-08-05T20:30:00'), durationS: 545,
    parsePct: 84.6, dps: 38900,
    fightPercentage: null, bossPercentage: null, lastPhase: null,
  },
  {
    reportCode: 'ProgMockRep2CcDd', fightId: 4, encounterId: 1085,
    job: 'Samurai', kill: false,
    startTimeMs: Date.parse('2026-07-28T20:05:00'), durationS: 95,
    parsePct: null, dps: null,
    fightPercentage: null, bossPercentage: null, lastPhase: 0,
  },
];
...
export const mockSidecar: Sidecar = {
  ...
  async listCharacterPulls(args, onProgress): Promise<CharacterPullsResult> {
    onProgress?.(10, 'Scanning jobs');
    await delay(350);
    if (args.reportCode) {
      // Pasted-report mode: hand back one report's rows for the merge path.
      return {
        pulls: MOCK_PULL_ROWS.filter((r) => !r.kill).map((r) => ({
          ...r, reportCode: args.reportCode as string, parsePct: null, dps: null,
        })),
        encounters: [], jobs: ['Machinist'],
        syncedAtMs: Date.now(), recentLimit: 0,
      };
    }
    onProgress?.(60, 'Scanning recent reports');
    await delay(350);
    const rows = [...MOCK_PULL_ROWS].sort((a, b) => b.startTimeMs - a.startTimeMs);
    return {
      pulls: rows,
      encounters: MOCK_ENCOUNTERS,
      jobs: ['White Mage', 'Samurai', 'Machinist'],
      syncedAtMs: Date.now(),
      recentLimit: args.recentLimit ?? 10,
    };
  },
