...
export type Req =
  ...
  | {
      id: string;
      kind: 'list_character_pulls';
      /** The Pulls screen's one-shot: every tier pull for this character,
       *  job-agnostic (ranked kills across all jobs + wipes from recent
       *  reports), merged newest-first. */
      lodestoneId: number;
      /** Identity join for wipe rows: report summaries know actors, not
       *  lodestone ids, so wipes are attributed by name (+ server). */
      characterName: string;
      server: string;
      /** Refresh button: bypass + replace the handler's session memo. */
      forceRefresh?: boolean;
      /** Wipe-scan depth in recent reports (default 10); "Load older pulls"
       *  re-requests at 25/50. */
      recentLimit?: number;
      /** Pasted-report mode: list THIS one report's tier fights (kills and
       *  wipes, no rankings) instead of the merged character list. */
      reportCode?: string;
    }
...
/** One tier encounter with the character's kill totals — the Pulls screen's
 *  catalog rows. Zero-kill encounters are synthesized so filters and the
 *  wipe-scan strip can list the whole tier (ultimates are prog-heavy).
 *  Source: sidecar/main.py::list_character_pulls. */
...
/** One row of the job-agnostic Pulls screen: a ranked kill (parse/dps set) or
 *  a wipe from a report summary (phase/progress set) — `kill` says which.
 *  Labels/meta lines are built client-side (src/views/pullsFormat.ts).
 *  Source: sidecar/main.py::_serialize_pull_row. */
export type PullRow = {
  reportCode: string;
  fightId: number;
  encounterId: number;
  /** The job THIS character played in the fight (internal spaced name). */
  job: string;
  kill: boolean;
  startTimeMs: number;
  durationS: number;
  /** Kills only (and null on pasted-report kills — summaries carry no parses). */
  parsePct: number | null;
  dps: number | null;
  /** Wipes only: phase-weighted % of the whole fight remaining. */
  fightPercentage: number | null;
  /** Wipes only: current boss's HP % remaining (display-only). */
  bossPercentage: number | null;
  /** Wipes only: phase the wipe happened in (0 = unphased/unknown). */
  lastPhase: number | null;
};

/** The Pulls screen's one-shot payload. In pasted-report mode `encounters`
 *  is empty and `recentLimit` 0 — the client merges `pulls` into its list.
 *  Source: sidecar/main.py::list_character_pulls. */
export type CharacterPullsResult = {
  pulls: PullRow[];
  /** The full tier catalog (kill totals summed across jobs; zero-kill
   *  encounters synthesized) — drives the filter pills + scan strip. */
  encounters: SetupEncounter[];
  /** Jobs with >= 1 row, in role order — the job filter pill's menu. */
  jobs: string[];
  syncedAtMs: number;
  recentLimit: number;
};
...
export interface Sidecar {
  ...
  /** The Pulls screen's one-shot: every tier pull for this character across
   *  ALL jobs (ranked kills + recent-report wipes), merged newest-first. The
   *  cold path is a multi-request fan-out — progress streams while it runs.
   *  Pass `reportCode` for the pasted-report mode instead. */
  listCharacterPulls(
    args: {
      lodestoneId: number; characterName: string; server: string;
      forceRefresh?: boolean; recentLimit?: number; reportCode?: string;
    },
    onProgress?: (pct: number, stage: string, tasks?: ProgressTask[],
                  meta?: ProgressMeta) => void
  ): Promise<CharacterPullsResult>;
